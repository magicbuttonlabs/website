...
**Also landed 2026-06-28 (visual/structure polish, after the language batch):** card copy tightened to the punchline (Builders Blueprint, AI Workshop, Foundation Build — teaser-length now; detail lives on the full pages); hero headline resized + `text-wrap: balance` for a clean two-line wrap, hero path-card icons enlarged and set inline with their labels; **offering section labels emphasized into a spine** — each now carries its matching hero icon (🎓 Enable / 🚀 Deploy & Build / 🔧 Support & Maintain) at pillar weight, visually linking the hero cards to the offering groups below.
**DECISION (2026-06-28) — homepage KEEPS the three-mode spine; do NOT collapse it.** The earlier-logged "collapse to two paths" was reconsidered and reversed for the homepage: Enable / Deploy & Build / Support & Maintain maps the customer's whole lifecycle and is a stronger story than a generic two-path frame, and Corrine's real objection (heaviness) was already solved by tightening the cards. So the **homepage carries the three-mode story**, and the **"two paths to win" simplification lives on `solutions.html`** instead. Corrine's "drop the subheaders" note was her reacting to them as standalone clutter, not tracking that they tie back to the hero panel — so emphasizing (not removing) them was the right call.
**Done 2026-06-28 (same session):**
- **`solutions.html` brought to consistency (Option B, no reframe).** Ethan chose to KEEP the three-mode spine on Solutions rather than reorganize it ("I don't think we need to reframe. It's duplicative and that's OK."). Solutions now mirrors the homepage's emphasized 🎓/🚀/🔧 spine, with the StoryBrand-flipped + tightened card copy, a "how to think about it" intro line, the pricing/stat meta lines kept as its "level deeper," and the new CTA-band copy. (Previews A and B were built to decide; A = reorganize-around-the-decision was declined.)
- **Solutions nav → hover dropdown — DONE.** Added dropdown CSS to `styles.css` and swept the dropdown markup across all 23 chrome-bearing pages (12 mains, 5 overviews, 6 blog posts). The menu lists all five offerings with their spine icons + an "All Solutions →" link; clicking "Solutions" itself still goes to the hub. Desktop = hover; mobile = items indented in the slide-down. Offering pages carry the active-on-Solutions variant, others the plain variant. 2-pagers excluded (own print chrome); `customer-journey-v2.html` stub left alone.
**Done 2026-06-28 — footer sweep (Custom Work + © year), all 23 chrome pages:**
- **Custom Work added to the footer Solutions column** so nav + footer now agree (5 offerings everywhere). Combined in one edit per page with the year bump below.
- **Site-wide © 2025 → 2026** on every chrome page's footer-bottom.
- **Footer-block variants confirmed during the sweep:** the footer is NOT byte-identical site-wide — the Connect column drifts. 19 pages carry the **3-link** Connect (LinkedIn / Get monday.com / Workshop Waitlist): all mains + all 6 blog posts + `custom-work-overview.html`. 4 primers carry only a **2-link** Connect (no Workshop Waitlist): `foundation-build-overview`, `customer-journey-overview`, `workshop-overview`, `ai-workshop-overview`. (`custom-work-overview` is the odd primer out — it was built on the 3-link main footer.)
**Open — remaining footer drift (separate from the sweep):**
- **Connect-column inconsistency.** The 4 older primers are missing the Workshop Waitlist link that every other page has. Decide whether to add it to those 4 (reconcile to 3-link everywhere) or drop it from the mains — a small, clean chrome sweep either way. Pre-existing; surfaced (not caused) by this work.
- **2-pagers untouched.** The 6 print 2-pagers have their own chrome and weren't part of the © sweep; if any carry a © 2025 it's still stale. Bumping them regenerates the PDFs — say the word and I'll sweep + rebuild.
**Parked — later / v2:**
